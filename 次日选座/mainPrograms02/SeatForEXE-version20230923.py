...
# ===================================== 开始挂机======================================== #
    ...
    # ==============================================进入倒计时=====================================
        # 提前十秒结束倒计时,下面开始请求场馆状态
        print('挂机结束,提前查询常用所在场馆的列表下标')
        # 这一步就是查常用场馆的下标,以及常用座位1号的一些信息
        ...
        libs = responseLib['data']['userAuth']['reserve']['libs'] # 选座中的所有场馆
        # 常用座位一号所在的馆室id 以及馆室名字信息获取
        ...
        oftenlibIndex = None
        for i in libs:
            # 如果查到的馆室的id和常用的一致,就获取这个场馆在返回的数据列表的下标
            if i['lib_id'] == oftenlibid:
                oftenlibIndex = libs.index(i)
                print(f"常用馆室:id:{i['lib_id']},name:{i['lib_name']},index:{oftenlibIndex}")
                break
        ...
        pass
        #
        # # 开馆前 查询主页信息 (0为空闲 1为被选;查询场馆是否还有剩余位置 num>=0 表示场馆还未满)
        # responseOftenSeats = response.post(url=url, json=home, verify=False).json()
        # # 获取预约信息
        ...
        open_num = 1
        while True:
            # 从选座中获取所有馆室的列表,这个请求是全新的请求,每次循环都会刷新场馆的is_open状态,直到刷到True就退出循环,开始去抢座
            try:
                responseLib = response.post(url=url, json=liblist, verify=False).json()
                libs = responseLib['data']['userAuth']['reserve']['libs']
                print(f'No.{open_num}次请求场馆是否开放,请稍稍候......')
                # 检测是否常用一号馆是否开馆,注意,下标用的是倒计时之前查的,是提前查好的
                if oftenlibid == libs[oftenlibIndex]['lib_id'] and libs[oftenlibIndex]['is_open'] == True:
                ...
                # 打印导致解码错误的源文本,
                print("返回数据异常的内容.text:"+responseLib.text)
                # 将错误写入日志中
                with open('errorlog.txt', 'a+', encoding='utf-8') as fp:
                    fp.write(f"{datetime.datetime.now()}报错信息:\n" + traceback.format_exc() + '\n'+'返回数据异常的内容:'+responseLib.text+'\n')
...
# ========================如果座位所在场馆未满,且座位未被选,则添加到列表中;如果说列表为空,则说明两个座位都没法选=========================   #
    ...
    startTime = time.time()
    print("\n====================================开始请求常用座位==========================================")
    # 上面确认常用馆室开馆后 重新查主页常用状态,重置常用座位状态!!!!!!
    ...
    # 初始化一个常用座位,如果确认有常用空闲,则会把这个常用的馆室id和座位号打包成请求需要的json参数
    ofseat = None
    for seat in AfteroftenseatsList:
        # 状态是1表示被选,0是未选 TODO 状态测试,记得改回 == 0
        if seat['status'] == 0:
...
# ===================================常用座位选座=============================================
    ...
    if ofseat is not None:
            ...
            if r_json['data']['userAuth']['reserve']['reserveSeat'] is not None:
                # 如果返回的是True,则选座成功
    ...
    if len(seats_expands) > 0 and ofseat is None:
        ...
        for data in seats_expands:
            ...
            if amount != 0:
                ...
                print(f'\n {libName}场馆剩余数:', seatsFree, " 这里的剩余座位数因为包括了一些没有名字的座位比如沙发,所以请以抢到的座位为准!")
    ...
    for i in [0,1]:
        ...
        for seat in seatsList:
            ...
            if seatStatus == 1 and status == False and Type == 1 and seatName is not None:
                ...
                # 判断请求是否通过
                if r_json['data']['userAuth']['reserve']['reserueSeat'] is not None:  # 如果返回的是True
...
def main():
    ...
    try:
        ...
        with open('newCookie.txt', 'w', encoding='utf-8') as fp:
            #  这里的 "更新的cookie为:" 这一段不要改,特别是 : 号,我用来断线的时候分割出cookie用的
            fp.write(f"{datetime.datetime.now()}更新的cookie为:"+configSet.get('header')['Cookie'])
            print('已经将初始的cookie保存到了newCookie.txt中!')
