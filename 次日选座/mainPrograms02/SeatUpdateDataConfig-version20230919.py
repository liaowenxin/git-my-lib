...
def saveCookie(cookieStr):
    ...
    '''
    #读取数据
    with open('../config02/User.json', 'r', encoding='utf-8') as fp:
       data = json.load(fp)
    ...
    #储存数据
    with open('../config02/User.json', 'w', encoding='utf-8') as fp:
...
def switchData():
    ...
    configDict = getConfig('../config02/User.json')
    ...
    config = getConfig(f'../config02/config.json')
...
def saveConfig():
    for content in switchData():
        with open(f'../config02/config.json', 'w', encoding='utf-8') as fp:
