...
def bookSeat(url, header, seats_expands, openTime, bookTime, directBook):
    ...
    while True:
        ...
        # 给常用单独配一个session重刷,这里把上面的response的请求头和cookies都设置到最新的session里面来
        ofseatsResponse = requests.session()
        # 设置新的头
        ofseatsResponse.headers = response.headers
        # 设置新的cookies
        ofseatsResponse.cookies = response.cookies
        ...
        responseOftenSeats = ofseatsResponse.post(url=url, json=home, verify=False).json()
        # 这个是开馆后的请求返回的常用座位信息列表
        AfteroftenseatsList = responseOftenSeats['data']['userAuth']['oftenseat']['list']
        # 常用一号位置
        firstSeat = AfteroftenseatsList[0]

        # TODO 这里我想测试顺延多久才可以正常轰炸, 设置开放后三秒内无限制轰炸常用一号,无限制重刷请求轰炸
        if openTime2 + 3 > time.time():
        ...
        print("=====================常用一号轰炸结束,没有抢到,继续访问常用二号====================\n")
        # 开馆之后0.5秒,看一下座位在不在,不在去
        ...
        if ofseats > 0 and ofseat is not None:
            ...
            # print(end - startTime)
            time.sleep(0.8)
            r = ofseatsResponse.post(url=url, json=ofseat, verify=False)
            r_json = r.json()
            # 判断请求是否通过 reserveSeat
            if r_json['data']['userAuth']['reserve']['reserveSeat'] is not None:
                # 如果返回的是True,则选座成功
            ...
            # # 默认不延迟
            # delayTime = 0
            # # 如果当前时间晚于开放时间,就设置最小的延迟,确保能选上
            # if openTime2 + 0.05 < time.time():
            #     delayTime = 0.75
            # time.sleep(delayTime)
    ...
    if len(seats_expands) > 0 and ofseats == 0:
        ...
        for data in seats_expands:
            ...
            if amount != 0:
                ...
                print(f'{libName}场馆剩余数:', seatsFree,
